...
describe('TTL Caching Examples', () => {
  ...
  it('should demonstrate tiered caching strategy', async () => {
    const config = await resolve({
      ...
      resolvers: [
        cached(mockSecretsProvider({ USER_SESSION_TOKEN: 'session-token' }), { ttl: TTL.short, staleWhileRevalidate: true }),
        cached(mockSecretsProvider({ DATABASE_PASSWORD: 'db-pass-tiered' }), { ttl: TTL.minutes5, staleWhileRevalidate: true }),
        cached(mockSecretsProvider({ APP_VERSION: 'v1.2.3' }), { ttl: TTL.hour, staleWhileRevalidate: false }),
      ],
    });
  ...
  it('should demonstrate production-ready tiered caching', async () => {
    const config = await resolve({
      ...
      resolvers: [
        cached(mockSecretsProvider({ JWT_SECRET: 'app-jwt', ENCRYPTION_KEY: 'app-enc' }), { ttl: TTL.hour, maxAge: TTL.day, staleWhileRevalidate: true, key: 'app-secrets' }),
        cached(mockSecretsProvider({ DATABASE_URL: 'https://prod-db.com', REDIS_URL: 'https://prod-redis.com' }), { ttl: TTL.minutes15, maxAge: TTL.hour, staleWhileRevalidate: true, key: 'database-creds' }),
        cached(mockSecretsProvider({ STRIPE_SECRET_KEY: 'stripe-prod', SENDGRID_API_KEY: 'sendgrid-prod' }), { ttl: TTL.minutes5, maxAge: TTL.minutes15, staleWhileRevalidate: true, key: 'api-keys' }),
      ],
      policies: {
