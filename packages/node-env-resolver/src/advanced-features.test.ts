...
import { dotenv, json as jsonResolver } from './resolvers';
import {
  string,
  stringArray,
  numberArray,
...
describe('Advanced Features', () => {
  ...
  describe('Array support - url[])', () => {
    it('should parse and validate multiple URLs', async () => {
      process.env.URLS =
        'https://api.example.com,https://cdn.example.com,http://localhost:3000';
          ...
          [
            {
              name: 'test',
              load: () =>
                Promise.resolve(process.env as Record<string, string>),
            },
            {
              URLS: urlArray(),
            },
  ...
  describe('SecretsDir support', () => {
    it('should read from secretsDir when no env var provided (global option)', async () => {
      ...
      fs.mkdirSync(secretsDir, { recursive: true });
      fs.writeFileSync(
        path.join(secretsDir, 'db-password'),
    ...
    it('should read from secretsDir when no env var provided (per-field option)', async () => {
      ...
      fs.mkdirSync(secretsDir, { recursive: true });
      fs.writeFileSync(
        path.join(secretsDir, 'api-key'),
        'api-key-from-secrets-dir',
      );
    ...
    it('should prefer env var path over secretsDir', async () => {
      ...
      fs.mkdirSync(secretsDir, { recursive: true });
      fs.writeFileSync(
        path.join(secretsDir, 'db-password-file'),
        'password-from-secrets-dir',
      );
    ...
    it('should convert SCREAMING_SNAKE_CASE to kebab-case for K8s', async () => {
      ...
      fs.mkdirSync(secretsDir, { recursive: true });
      fs.writeFileSync(
        path.join(secretsDir, 'my-secret-key'),
      ...
      const config = await resolveAsync({
        resolvers: [
          [
            {
              name: 'test',
              load: () => Promise.resolve({ TEST_FILE: relativePath }),
            },
            {
              TEST_FILE: file(),
            },
      ...
      const config = await resolveAsync({
        resolvers: [
          [
            {
              name: 'test',
              load: () => Promise.resolve({ NESTED_FILE: relativePath }),
            },
            {
              NESTED_FILE: file(),
            },
  ...
  describe('Resolver file path resolution', () => {
    ...
    it('should resolve json file paths relative to current working directory', async () => {
      ...
      fs.writeFileSync(
        jsonFile,
        JSON.stringify({ JSON_TEST: 'value-from-json' }),
      );
