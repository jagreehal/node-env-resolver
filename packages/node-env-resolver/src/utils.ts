    ...
    async load() {
      ...
      if (staleWhileRevalidate && !cache.refreshPromise) {
        ...
        const refreshPromise = Promise.resolve(
          resolver.load ? resolver.load() : Promise.resolve({}),
        );
...
>(
  ...
  const result = { ...config } as TConfig & {
 ...
 * ```
 */
export function withNamespace(resolver: Resolver, namespace: string): Resolver {
