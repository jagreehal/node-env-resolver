  ...
  it('works with Zod-like schema', async () => {
    const schema = {
      parse(input: unknown) {
        ...
        return {
          NODE_ENV: String(obj.NODE_ENV ?? 'test'),
          PORT: Number(obj.PORT ?? 3000),
        };
      },
    };
      ...
      resolvers: [
        {
          name: 'mock',
          async load() {
            return { NODE_ENV: 'development', PORT: '4000' } as Record<
              string,
              string
            >;
          },
...
describe('Connection String Types', () => {
      ...
      const config = await resolveAsync({
        resolvers: [
          [
            mockProvider({
              DB_URL:
                'postgres://user:password@host.example.com:5432/database?sslmode=require',
            }),
            {
              DB_URL: postgres(),
            },
    ...
    it('accepts HTTP URL with path and query', async () => {
      const config = await resolveAsync({
        resolvers: [
          [
            mockProvider({
              API_URL: 'https://api.example.com/v1/users?limit=10',
            }),
            {
              API_URL: http(),
            },
      ...
      const config = await resolveAsync({
        resolvers: [
          [
            mockProvider({
              API_URL:
                'https://user:pass@api.example.com:443/v1?key=value#section',
            }),
            {
              API_URL: https(),
            },
  ...
  describe('resolveAsync() tuple API', () => {
    it('resolves with single provider tuple', async () => {
      ...
      const config = await resolveAsync({
        resolvers: [[provider1, { FOO: string(), PORT: number() }]],
      });
    ...
    it('supports options as last argument', async () => {
      ...
      const config = await resolveAsync({
        resolvers: [
          [
            provider1,
            { REQUIRED: string(), OPTIONAL: string({ optional: true }) },
          ],
        ],
        options: { strict: true },
      });
  ...
  describe('resolveAsync() tuple API (sync)', () => {
    it('resolves synchronously with tuples', async () => {
      ...
      const config = await resolveAsync({
        resolvers: [[provider1, { FOO: string() }]],
      });
